...
def process_profile(linkedin_url, api_key, use_mock, selected_model):
    ...
    Returns:
        Initial facts about the profile and a session ID.
    """
    try:
        ...
        if not profile_data:
            return (
                "Failed to retrieve profile data. Please check the URL or API key.",
                None,
            )
...
def chat_with_profile(session_id, user_query, chat_history):
    ...
    if not session_id:
        return chat_history + [
            [user_query, "No profile loaded. Please process a LinkedIn profile first."]
        ]

    if session_id not in active_indices:
        return chat_history + [
            [user_query, "Session expired. Please process the LinkedIn profile again."]
        ]
...
def create_gradio_interface():
    ...
    with gr.Blocks(title="LinkedIn Icebreaker Bot") as demo:
        ...
        gr.Markdown(
            "Generate personalized icebreakers and chat about LinkedIn profiles"
        )
