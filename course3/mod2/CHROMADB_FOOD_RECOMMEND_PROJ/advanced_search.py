...
def perform_cuisine_filtered_search(collection):
    ...
    cuisine_choice = input(
        "Enter cuisine number (or cuisine name): "
    ).strip()
    ...
    display_search_results(
        results,
        f"Cuisine-Filtered Results ({cuisine_filter})"
    )
...
def perform_calorie_filtered_search(collection):
    ...
    print(
        f"\n🔍 Searching for '{query}'"
        + (
            f" with max {max_calories} calories..."
    ...
    display_search_results(
        results,
        f"Calorie-Filtered Results ({calorie_text})"
    )
...
def perform_combined_filtered_search(collection):
    ...
    max_calories_input = input(
        "Enter maximum calories (optional): "
    ).strip()
    ...
    if cuisine_filter:
        filter_description.append(
            f"cuisine: {cuisine_filter}"
        )

    if max_calories:
        filter_description.append(
            f"max calories: {max_calories}"
        )
    ...
    display_search_results(
        results,
        f"Combined Filtered Results ({filter_text})"
    )
...
def run_search_demonstrations(collection):
    ...
    for i, demo in enumerate(demonstrations, 1):
        ...
        if demo['cuisine_filter']:
            filters.append(
                f"Cuisine: {demo['cuisine_filter']}"
            )

        if demo['max_calories']:
            filters.append(
                f"Max Calories: {demo['max_calories']}"
            )
...
def display_search_results(
    ...
    for i, result in enumerate(results, 1):
        ...
        if show_details:
            ...
            print(
                f"   🔥 Calories: "
                f"{result['food_calories_per_serving']}"
            )
            print(
                f"   📝 Description: "
            ...
            print(
                f"   {i}. {result['food_name']} "
                f"({score_percentage:.1f}% match)"
            )
